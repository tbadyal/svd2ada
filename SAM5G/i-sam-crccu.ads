...
package Interfaces.SAM.CRCCU is
   ...
   type MR_BITORDER_Field is
     (--  CRC computation is performed from the most significant bit to the least
--  significant bit
      MSBFIRST,
      --  CRC computation is performed from the least significant bit to the most
--  significant bit
      LSBFIRST)
     with Size => 1;
   ...
   type CRCCU_MR_Register is record
      ...
      --  unspecified
      Reserved_8_16  : Interfaces.SAM.UInt9 := 16#0#;
      --  Precomputation Bit Swap Operation of the CRC
      BITORDER       : MR_BITORDER_Field := Interfaces.SAM.CRCCU.MSBFIRST;
      --  unspecified
