...
package Interfaces.STM32.SDIO is
   ...
   type POWER_Register is record
      --  PWRCTRL
      PWRCTRL       : POWER_PWRCTRL_Field := Interfaces.STM32.SDIO.Power_OFF;
      --  unspecified
...
--  edge of SDMMC_CK.
   ...
   type CLKCR_Register is record
      ...
      --  Wide bus mode enable bit
      WIDBUS         : CLKCR_WIDBUS_Field :=
                        Interfaces.STM32.SDIO.Bus_Wide_1b;
      --  SDIO_CK dephasing selection bit
      NEGEDGE        : CLKCR_NEGEDGE_Field :=
                        Interfaces.STM32.SDIO.Edge_Rising;
   ...
   type CMD_Register is record
      ...
      --  Wait for response bits
      WAITRESP       : CMD_WAITRESP_Field :=
                        Interfaces.STM32.SDIO.No_Response;
   ...
   type DCTRL_Register is record
      ...
      DTDIR          : DCTRL_DTDIR_Field :=
                        Interfaces.STM32.SDIO.Controller_To_Card;
      --  Data transfer mode selection 1: Stream or SDIO multibyte data
      --  transfer.
      DTMODE         : DCTRL_DTMODE_Field := Interfaces.STM32.SDIO.Block;
      --  DMA enable bit
      DMAEN          : DCTRL_DMAEN_Field := 16#0#;
      --  Data block size
      DBLOCKSIZE     : DCTRL_DBLOCKSIZE_Field :=
                        Interfaces.STM32.SDIO.Block_1b;
      --  Read wait start
